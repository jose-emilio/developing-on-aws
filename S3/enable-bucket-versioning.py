import boto3
from botocore.exceptions import ClientError
import sys

#HeadBucket sólo está disponible en la API de bajo nivel de S3
def existeS3Bucket(nombreBucket):
    try:
        session = boto3.Session(profile_name='lab')
        client = session.client('s3')
        client.head_bucket(Bucket=nombreBucket)
        respuesta = 'El bucket existe'
    except ClientError as e:
        match e.response['Error']['Code']:
            case '404':
                respuesta = 'El bucket '+nombreBucket+' no existe'
            case '403':
                respuesta = 'No se tienen permisos para acceder al bucket '+nombreBucket
            case _:
                respuesta = 'Error de cliente: '+e.response['Error']['Code']
    finally:
        return respuesta

def versionarBucket(nombreBucket):
    try:
        comprobar = existeS3Bucket(nombreBucket)
        if comprobar != 'El bucket existe':
            raise ClientError
        session = boto3.Session(profile_name='lab')
        client = session.client('s3')
        client.put_bucket_versioning(
            Bucket=nombreBucket,
            VersioningConfiguration={
                'Status':'Enabled'
            }
        )
        respuesta = 'Se habilitó el versionado para el bucket '+nombreBucket
    except ClientError as e:
        respuesta = 'Error de cliente: '+e.response['Error']['Code']
    return respuesta

if len(sys.argv)!= 2:
    print('Uso: enable-bucket-versioning <nombre-bucket>')
else:    
    respuesta = versionarBucket(sys.argv[1])
    print(respuesta)